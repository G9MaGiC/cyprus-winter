...
| Trust | `/trails/[id]` — “Latest from hikers” vs static conditions (`SectionCard`) | 2 | 2 | 2 | When `getLatestReportsByTrail` returns data, payoff is high; without reports, falls back to copy/static—**density varies by trail**. |
...
| Step | Route / surface | R | S | P | Notes |
|------|-----------------|---|---|---|--------|
| Discover | `/discover` filters | 2 | 1 | 1 | **Family-specific affordance** often copy-deep; no dedicated “family” filter chip visible in flow docs—verify filter taxonomy in `DiscoverFilterBar`. |
...
| **Total signal** | | | | | **Family “amazement” needs more Discover/home specificity** (filters or curated strip). |
...
|----|----------|---------|-----|----------------|
| G1 | **P0** | Claire | Plan share / export lacks “story-grade” preview (OG, title, one-line trip summary for recipients). | `/plan`, `PlanShareBar`, metadata |
| G2 | **P0** | Anders | Conditions UX degrades when no recent reports—needs honest empty-state + CTA to report (trail list + detail). | `/trails`, `/trails/[id]` |
| G3 | **P1** | Family | Discover/home lack obvious family-oriented entry (filter strip, curated row, or copy block). | `/`, `/discover` |
| G4 | **P1** | Nadia | AI suggestion chips under-index Digital Nomad hook from MESSAGING (“Work from 16°C…”). | `AIAssistant.tsx` |
| G5 | **P1** | Bleisure | Validate `short-stay` template density end-to-end; may need fewer clicks from `/airport` CTA to filled Plan. | `/plan`, `ITINERARY_TEMPLATES` |
| G6 | **P2** | Local | Events index freshness perception (seasonal copy, “updated” cues). | `/events` |
| G7 | **P2** | All | `docs/USER_ICPS.md` vs `docs/ICPS.md` priority wording differs for Anders—align docs to avoid roadmap drift. | Docs |
...
| Digital Nomad | Work from 16°C. Weekend trails, villages, heritage. | Home/meta “sixteen degrees”; AI chips weak on “work / weekend” phrasing | Partial |
| Winter Sun Family | Mild weather. Beaches, ruins, markets. Family-friendly Cyprus. | AI Discover suggestion “Family-friendly places in winter” | Partial — **hero not family-specific** |
...
1. Review **G1–G2** in sprint planning (Claire share + Anders empty states).
2. Align **docs/ICPS.md** vs **USER_ICPS.md** Anders priority with product once per quarter.

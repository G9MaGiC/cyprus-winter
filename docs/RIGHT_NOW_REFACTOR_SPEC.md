# Right Now Location/Region Refactor — Full Implementation Spec

Refactor the "Use my location" / "Pick a region" flow in RightNowNearYou with complete, copy-paste-ready code.

---

## 1. `src/data/regions.ts` — Add `getRegionShortLabel`

Add this function (and export):
...
## 2. `src/components/LocationActionButtons.tsx` — New file
...
## 3. `src/components/RegionPickerChips.tsx` — New file
...
export default function RegionPickerChips({ onSelect, onUseLocation }: RegionPickerChipsProps) {
  return (
    <>
...
## 4. `src/hooks/useRightNowFeed.ts` — New file
  ...
  const handleDistanceChange = useCallback(
    (mode: DistanceMode) => {
      ...
      if (coords && (state === "loaded" || state === "empty")) {
        fetchFeed(coords.lat, coords.lng, mode === "less", sourceMode, selectedRegion);
      }
    },
...
## 5. `src/app/_home/RightNowNearYou.tsx` — Refactored
...
| File | Action |
|------|--------|
| `src/data/regions.ts` | Add `getRegionShortLabel` |
| `src/components/LocationActionButtons.tsx` | Create |
| `src/components/RegionPickerChips.tsx` | Create |
| `src/hooks/useRightNowFeed.ts` | Create |
| `src/app/_home/RightNowNearYou.tsx` | Refactor to use new pieces |

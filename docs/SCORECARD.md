...
| **Core funnel** | Discover → Plan → Book E2E green; Plan server shell + client leaf | `npm run test:e2e:gate:ci`, `src/app/(padded)/plan/page.tsx` | **5/5** — CI Core Funnel + E2E Full green on `ebbf609` |
...
| **Test & CI** | lint, typecheck, 600+ unit tests, E2E gate, build | `.github/workflows/ci.yml` | **5/5** — 622 unit tests; Core Funnel + E2E Full green on `ebbf609`; Actions on Node 20.19.0 |

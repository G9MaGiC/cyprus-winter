...
| Dimension | 5/5 criteria | Verify | Status (2026-05-31) |
|-----------|----------------|--------|---------------------|
...
| **Core funnel** | Discover → Plan → Book E2E green; Plan server shell + client leaf | `npm run test:e2e:gate:ci`, `src/app/(padded)/plan/page.tsx` | **5/5** — CI Core Funnel + E2E Full green on `6462605` |
...
| **Test & CI** | lint, typecheck, 570+ unit tests, E2E gate, build | `.github/workflows/ci.yml` | **5/5** — 572 unit tests; 5 core funnel + 7 UX E2E specs in gate |
| **i18n / SEO** | 7 locales validate; scan clean; hreflang for all locales | `npm run i18n:validate`, `npm run i18n:scan --fail`, `npm run build` | **5/5** — 1848 keys × 7 locales |
...
| `6462605` | E2E plan-offline aligned with overlay/footer behavior |

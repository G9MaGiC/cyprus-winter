...
## 0.0 Final state (2026-09-05, after batch 56)
...
Where the branch ended up, 56 batches later (batches 1–46 merged in PR #227, 47–48 in PR #231, 49 in PR #232, 50–51 in PR #233, 52–55 in PR #234; batch 56 continues on the restarted branch):
...
- **The audit's top project (AUD-10) shipped in full:** the entire curated content register renders natively in all 7 locales — 71 wineries, 58 attractions, 105 trails, 78 secret gems, 305 `bestFor` tags; catalog 7×2312 → 7×3578, EN data layer and JSON-LD contracts intact.
- **Eight adversarial self-review passes** (batches 11, 31, 34, 36, 42, 44, 49, 56) caught and fixed 47 defects in this branch's own code before any human review.
- **A register blind spot found and named (batch 56):** the audit's AUD-10 class list came from what the i18n gate could see, and `i18n:scan` deliberately skips `src/data/` — so six further data modules (weather month descriptions, the `/airport` transport guide, events, restaurants, nature excursions, official cycling routes, wine-route intros) still render English prose inside localized frames. Real but un-enumerated by AUD-10; queued as the next arc, not silently absorbed into "complete".
...
- **Handoff ready:** `docs/i18n-review/` holds per-locale native-review sheets for every ◇ translation since the audit baseline (`main@ca827e6`, pinned in the export script as of batch 56 — a merge-base-scoped diff had silently dropped 19 keys on a branch restart), CI-guarded for freshness.
- **Four parked product decisions resolved with the owner and shipped (batches 38–41):** the `/team` fabricated roster replaced by the honest "How we choose places" page (§5.2), the workation restructure (AUD-14), the tier-1 headline unification (AUD-19), and guest-side cancel (the migration-008 payoff).
- **Per-locale advisory links (batch 43):** the travel-trust strip links each locale's own authority once the sources were verified — the AUD-25 quick fix.
- **Still open, none code-shaped:** real legal mailbox (AUD-67), legal "last updated" (AUD-72), the partner onboarding *contact channel* (AUD-81/67 residual — the honest `/partner/join` page shipped batch 48; a real mailbox still gates the direct channel), per-guide price quotes (AUD-23 residual — the honest association-range signal shipped batch 47), family-lane curation (AUD-58), AUD-25's remaining half (alert-time crisis behavior needs an alert source; the `/book/*` strip question was closed by design in batch 50 — booking pages keep their focused `BookingTrustStrip`, with the advisory strip one tap away on home, `/plan` and `/bookings`), the rest of the §5 known-open ledger (partner reachability, email domain, supply-gated content, photos, legal sign-off), and the native ◇ review itself. The AUD-10 content register is finally whole: 71 wineries, 58 attractions, 105 trails and — as of batch 55 — all 78 secret gems render natively in every locale.

---

## 0.1 Remediation status (fix pass, same day / same branch)

> *Audit-time snapshot (round 1, 2026-08-31). The "deliberately not fixed" list below describes that first pass only — most of it was closed by later batches: AUD-10 (batches 5–32), AUD-26 (batch 30), AUD-27 (batches 6–7), AUD-11/46/48/56/59 (batch 9), AUD-21 (batch 8), AUD-18 (batch 6). AUD-14 and AUD-19 were later resolved with the owner and shipped (batches 39–40). Still genuinely open as supply items: AUD-23's per-guide quotes (the attributed association-range signal shipped batch 47), AUD-58 (family curation) and the §5 ledger's supply rows; AUD-25's link map shipped once its sources were verified (batch 43) — only its alert-time and `/book/*` halves remain; AUD-49's practical surface shrank to near-zero once AUD-10 completed (native content replaced the EN fallback on /he).*
...
| AUD-14 | Workation template contradicts its own "Remote weekdays… wifi cafés" promise — every day gets 2 tourist stops, e.g. a 3h hike + winery on a workday — **✅ batch 39 (user-greenlit):** weekdays 1–5 now carry one nearby after-work stop each (museum, salt-lake walk, coast, village evening), the two-stop full days moved to the weekend (Artemis+Tsiakkas; Paphos mosaics+Kolios); copy updated ×7; a guard test enforces weekday lightness | `[S]` `src/data/itinerary-templates.ts:107-123` | Nadia | n |
...
| AUD-19 | Tier-1 home headline is a different copy generation: el/de/pl carry "Escape the cold…" while en/he/ro/fr carry "A quieter side of the island."; the editorial-drift gate guards beta locales only — intent unverifiable, needs an editorial decision either way — **✅ batch 40 (user-greenlit):** el/de/pl unified on the current EN generation («Η πιο ήσυχη πλευρά του νησιού.» / "Die ruhigere Seite der Insel." / "Spokojniejsza strona wyspy.") and `home.headline` pinned in the tier-1 drift maps so the generations can't silently diverge again | `[S]` `messages/{el,de,pl}.json` `home.headline` vs en/he/ro/fr | Local, DE, PL | y (decide + extend gate) |
...
| AUD-23 | No price signal on guide booking or plan (winery-only "from €" shipped); PL-01's budget lens unanswered — **✅ batch 47 (honest-range half):** guide hub, guide detail and the plan guide bar now carry an indicative, attributed range ×7 (€100–150 half-day / €150–400 full day, Cyprus Tourist Guides Association figures via FEG; "your guide confirms the final quote") — no per-guide prices fabricated; true per-guide quotes remain with the guides | `[S]` `src/data/guides.ts:3-22` (no guest price field) | PL-01, RO-01 | y (guides) |
...
| AUD-25 | Crisis posture is a static UK-centric strip: gov.uk advisory link hard-coded for all 7 locales; no alert-time behavior; strip absent from /book/* — **✅ batch 43 (link-map half):** each locale now links its own verified authority (Auswärtiges Amt / France Diplomatie / MSZ / MAE / Israel's NSC Cyprus page / Greek MFA Cyprus mission; EN keeps the FCDO) via `src/lib/advisory-links.ts` + `travelTrust.advisory` ×7; alert-time behavior (needs an alert source) stays open; strip-on-`/book/*` closed by design (batch 50 decision: booking pages keep their focused `BookingTrustStrip`, advisory links stay one tap away on home, `/plan` and `/bookings`) | `[S]` `src/components/travel/TravelTrustStrip.tsx:30` | CRISIS-01, IL-01 | y (per-locale link map) |
...
| 2 | Fabricated team roster on `/team` as the trust pillar | BUG-346 open decision | ✅ batch 38 — decision made (user chose the honest page): roster and `src/data/team.ts` removed; `/team` now carries "How we choose places" ×7 locales (winter verification, cited data, verified-never-paid partners, corrections path); nav relabeled ×7; `PRODUCT_DEEP.md` trust line now cites the transparent curation method |
...
| AUD-65 | Med | Soft 404s: every invalid dynamic slug (7 routes verified) serves the not-found page with HTTP 200 — streaming shell commits status before `notFound()` | ✅ batch 35: proxy-level slug validation. The earlier experiment stands (`dynamicParams=false` is inert while the root layout renders dynamically), but a URL matching **no** route returns a real 404 with the styled page — so `src/proxy.ts` now rewrites "known family + unknown slug" onto a non-matching sub-path before rendering (`invalid-slug-404.ts` + generated id sets mirroring each page's `generateStaticParams`, guard-tested). Verified live: all 7 families ×prefixed/unprefixed → HTTP 404 + styled page + full security headers; valid pages untouched. The locale-detection redesign is no longer needed for 404 correctness |
| AUD-66 | Med | `/install` is a developer deployment guide shipped as a public 7-locale route; `sitemap.ts` listed it while robots+meta forbid indexing | ✅ batch 51: the route is now a guest-facing "save the app" guide ×7 (benefits card + Android/iOS/desktop steps on the `/partner/join` template; claims limited to what `sw.js`/the manifest actually do; no `t.rich` — the BUG-350 class stays closed) and a first-class public page again: registry `robots` override dropped, `robots.ts` disallows removed, sitemap entry restored at 0.3/monthly. Rider: `src/app/apple-icon.png` so iOS home screens get a real touch icon |
...
| AUD-80 | Med | Server `VALIDATION_ERROR` genericized to "Something went wrong" — dropped the actionable reason; EN passthrough when a code was unmapped | ✅ per-code messages incl. `NOT_FOUND`/`IDEMPOTENCY_CONFLICT`, no EN passthrough (falls to localized generic); batch 50 closed the deferred half: `VALIDATION_ERROR` now names the failing field (`details[].field`), and the form marks that input (`aria-invalid` + localized inline marker ×7) and moves focus to it |
...
| AUD-99 | Med | Finite data enums (activities, difficulty, route type) interpolated raw into localized sentences → "Ιδανικό για shopping and crafts", "7 km szlak easy" | ✅ difficulty labels + routeType chips (batch 3); bestFor vocabulary in batch 32 — 305 tokens ×7 via `data.bestFor.{slug}` display overlay (data/search/matching/JSON-LD keep the EN tokens); `activities` renders on no surface, so no residual remains |
...
| AUD-107 | Low | he register split singular-masculine vs plural-neutral on one screen, incl. new strings | ✅ new-batch strings pluralized as shipped; the app-wide unification landed as batch 28 (65 edits: plural imperatives + geresh/gershayim + «» quotes); confirmation folds into the native ◇ review |
...
| AUD-113 | Med | Soft-404 (= AUD-65, SEO surface) | ✅ batch 35 (see AUD-65): invalid slugs now answer HTTP 404, so crawlers stop indexing phantom pages |
...
**Batch 21 (2026-09-01, twenty-first commit) — AUD-10 slice 12: the winery class is complete.** The final 21 estates join the overlay — ezousa, hadjipavlou, olympus-winery, minous, tria-elit, ayii-anargyri, pittali, papaioannou, karseras, evangelou, chrysoroyiatissa-winery, loel, etko-olympus, revecca, mallia, monolithos, antoniades, ayios-savas, povis, anama, linos. 56 EN strings mirrored, 336 ◇ translations, with the heaviest shared-line reuse yet (ten established recurring lines carry ~40% of the fields). **Every winery in the catalog — 71 of 71 — now serves its tasting info, winter tips, hours and logistics natively in all 7 locales.** Zero code changes; tier-1 drift maps seeded. Catalog 7×2869.

**Batch 22 (2026-09-01, twenty-second commit) — AUD-10 slice 13: trail content overlay pilot.** The third and final content class opens: new overlay machinery mirroring the winery/attraction pattern (`trail-content-ids.ts`, server-only `trail-content.ts`, guard suite ×7 locales) wired into the trail detail page — `winterNotes`, `winterSafety` and `localSecret` render per-locale while metadata and JSON-LD keep the EN base. Pilot slice: the 10 flagship winter trails (petra-tou-romiou, artemis, olympus-summit, caledonia-falls, adonis, madari-ridge, atalante, avakas-gorge, panagia-araka-stavros, cape-greco) — 30 EN strings mirrored, 180 ◇ translations, with the safety register preserved in every locale (microspikes/crampons, whiteout, flash-flood and Akamas-ordnance warnings, Emergency: 112). Tier-1 drift maps seeded. Catalog 7×2899. Remaining in the class: 95 trails on the established zero-code path.

**Batch 23 (2026-09-01, twenty-third commit) — AUD-10 slice 14: next 12 trails.** caledonia-alternative, horteri, millomeris-falls, aphrodite, almirolivado, mnimata-piskopon, kampos-tou-livadiou, kykkos-konizi, smigies, trooditissa-phini, kannoures-agios-nikolaos, loumata-ton-aeton join the trail overlay on the zero-code path — 30 EN strings mirrored, 180 ◇ translations, safety register intact (microspikes on Horteri, do-not-hike-alone on Almirolivado and Kykkos–Konizi, transport-at-both-ends on the linear routes, Emergency: 112). Tier-1 drift maps seeded. Trail coverage 22 of 105. Catalog 7×2929.

**Batch 24 (2026-09-01, twenty-fourth commit) — AUD-10 slice 15: next 15 trails + deterministic chat test.** livadi-trail, stavros-tis-psokas, venetian-bridges, chrysovrysi, moni-fylagra, aphrodite-cape-greco, ezousa-valley, machairas-forest, vretsia-roudias, persephone, kryos-potamos-loop, e4-troodos-platres, sea-caves-anargyroi, pissouromoutti, chorteri join the trail overlay — 30 EN strings mirrored, 180 ◇ translations (all 2-field records; the winterSafety class was fully covered by slice 14). Tier-1 drift maps seeded. Trail coverage 37 of 105. Catalog 7×2959. Also: `api/chat/unavailable.test.ts` flaked once more despite the batch-17 unique-IP fix — the in-memory rate limiter is worker-process state shared across test files, so the test now stubs the limiter to always allow, making the 503 provider-unavailability path deterministic by construction.

**Batch 25 (2026-09-01, twenty-fifth commit) — AUD-10 slice 16: next 17 trails, past the halfway mark.** agioi-anargyroi-circular, kavos-trail, prodromos-dam-stavroulia, troodos-visitor-centre, politiko-machairas, symvoulas, ariadni, prodromos-zoumi, vavatsinia-honeybee, vouni-panagias, agia-varvara-stavrovouni, konnoi-cyclops, cape-aspro, treis-elies, gnafkio, kionia-profitis-elias, psilo-dentro-pouziaris join the trail overlay — 34 EN strings mirrored, 204 ◇ translations, cautionary lines intact (stay-on-path on the Cape Greco cliffs, fit-hikers-only on the Marathasa descents, wet-steps on Stavrovouni). Tier-1 drift maps seeded. **Trail coverage 54 of 105 — past halfway.** Catalog 7×2993.

**Batch 26 (2026-09-01, twenty-sixth commit) — AUD-10 slice 17: next 17 trails.** mesa-potamos, stavrovouni-trail, argakas-dam, karvounarka, moutti-anemwn, agia-tilliria, kastrovounos, lazanias-fikardou, kalevounari, germasogeia-weir, kalopanagiotis-oikos, dymes-pelendri, fikardou-archontides, selladi-trypilos, polystypos-hazelnut, archangelos-mylos-rodous, agros-kato-mylos join the trail overlay — 34 EN strings mirrored, 204 ◇ translations; the "Official outside-forest trail…" line (9×) reuses one translation per locale. Tier-1 drift maps seeded. Trail coverage 71 of 105. Catalog 7×3027.

**Batch 27 (2026-09-01, twenty-seventh commit) — AUD-10 slice 18: the trail class — and the content register — are complete.** The final 34 trails join the overlay (panagia-tis-amasgou through panagia-agioi-saranda: the Larnaca salt-lake flamingo walk, the Lefkara lace path, the Agia Irini gorge, Panthea, Pentadaktylos, the Protaras coastal churches, and the village connectors). 68 EN strings mirrored, 408 ◇ translations. **All three AUD-10 content classes are done: 71 wineries, 58 qualifying attractions, 105 trails — the entire Discover/Trails decision surface renders natively in all 7 locales**, with EN base and JSON-LD contracts intact throughout. Tier-1 drift maps seeded. Catalog 7×3095.

**Batch 28 (2026-09-01, twenty-eighth commit) — he register sweep closed.** A programmatic sweep of all 3,095 Hebrew strings. The data.* catalog audited clean (plural imperatives, LTR isolates and Hebrew punctuation consistent throughout — the session's discipline held). The app copy carried two debt classes, both fixed (65 edits): 36 masculine-singular imperatives (skip links, aria labels, buttons: בחר/הוסף/דלג/גלול/צפה/נקה/…) moved to the gender-inclusive plural-imperative register — one string even mixed both forms in a single sentence — and ASCII apostrophes/quotes inside Hebrew abbreviations normalized to geresh/gershayim (ק״מ, קמ״ש, סופ״ש, דוא״ל, מ׳, ש׳, וכו׳, צ׳אט) with quoted terms unified to «…». `editorial-he.json` re-pinned for the 5 edited pinned keys. No key-count change.

**Batch 29 (2026-09-01, twenty-ninth commit) — migration 008 lands: booking locale + trail_id persisted.** `supabase/migrations/008_booking_locale_trail.sql` adds nullable `locale` and `trail_id` columns to `public.bookings` (the add-column pattern 003 established); `createBooking` persists both, `rowToBooking` and the explicit selects surface them. The two schema gaps flagged in the fix-pass close: **per-locale status emails** — partner confirm/decline now emails the guest in their own language (the ×7 `email.status.*` copy shipped in batch 8 only ever lacked a persisted locale; legacy rows keep the default-locale fallback) — and **durable trail choice** (AUD-86): DB-sourced booking rows now carry `trailId` themselves instead of leaning on the client's local-storage merge (which still covers legacy rows). Unit-guarded on both ends (creation persists locale; partner PATCH asserts the localized send). The guest-side cancel *action* remains the documented product decision — this lands the data model it needs.

**Batch 30 (2026-09-01, thirtieth commit) — migration 009: durable partner overlays (AUD-26 closed).** Partner-edited winter hours/hero image previously lived in a per-instance Map — every cold start dropped them and no two serverless instances agreed. `supabase/migrations/009_partner_overlays.sql` adds the `partner_overlays` table (RLS on, service-role access); `setPartnerOverlay` writes through (durable upsert first — a failure now surfaces as 503 from the profile PATCH instead of a silent memory-only edit), and a TTL'd fail-open `ensurePartnerOverlaysLoaded()` hydrates the cache at every overlay-consuming server entry point (wineries/discover/wine-route/booking pages + partner profile GET), so the synchronous read API and all its consumers keep their shape. Dev/tests without Supabase keep the exact previous in-memory behavior.

**Batch 31 (2026-09-01, thirty-first commit) — second adversarial review pass, 7 findings fixed.** A high-effort self-review of everything since batch 11 (the three overlay systems, migrations 008/009, the booking changes) surfaced seven defects, all remediated: (1) batch 30's `partner-overlay.ts` Supabase import reached the client bundle via `AttractionCard → place-card-hours` — the module split into a client-safe cache/read half and a server-only store (`partner-overlay-store.ts`), verified post-build (supabase-js only in the pre-existing lazy auth chunk; vitest now stubs `server-only` so such modules stay testable); (2) the booking form showed the full success screen on the honeypot's `200/stored:false` response — now a localized error; (3) the lean /discover projection dropped `tastingInfo`, losing the hours preview for ~19 wineries — restored; (4) raw `partnerEmail` was serialized into the public RSC payload — replaced by a server-computed `partnerVerified` boolean; (5) `applyPartnerOpeningHours` now recomputes `hoursCallAhead` from the hours it swaps in, so the badge matches what is shown (shared `isCallAheadHours` extracted to `call-ahead.ts`); (6) /discover localized shared items 2–3× across overlapping sections (~470 awaited calls) — each unique item now localizes once; (7) the plan removal SR announcement said the EN name while the card showed the localized one — now `getLocalizedName` throughout.

**Batch 32 (2026-09-02, thirty-second commit) — AUD-99 closed: the bestFor vocabulary localizes.** The audience/context tags were the last raw-EN class on localized decision surfaces — the discover detail "Great for" row rendered EN tokens under a localized heading, the whyNow hero bullet interpolated `join(" and ").toLowerCase()` EN tokens into every locale's sentence frame (the register's own "Ιδανικό για shopping and crafts" example), and both book/winery chip surfaces showed raw tags. All four sites now localize per token through a `data.bestFor.{slug}` message overlay: 305 distinct tokens collected from all five data modules (attractions 117, wineries 115, restaurants 72, activity places 31, itinerary templates 22 — deduped by slug, zero collisions), 1,830 new ◇ translations, catalog 7×3400. The tokens stay EN everywhere they act as matching keys — search index, personalization scoring, concierge ranking, AI context, JSON-LD, and React keys — the overlay is display-time only, per the BUG-110 pattern. Sentence joining is locale-correct (`Intl.ListFormat`; Hebrew fuses "ו" directly onto the last label because ICU's "ו-" form is wrong before a Hebrew word; EN keeps the legacy lowercased output byte-for-byte). Grammar-frame fixes rode along: the pl whyNow frame moved to a colon form ("Najlepsze dopasowanie: {types}.") since "Najlepsze dla" governs the genitive, which chip-form labels cannot satisfy, and el labels are authored case-neutral so one form serves the "Ιδανικό για" frame and the chips. Guarded end-to-end: `best-for.test.ts` walks the live data modules and asserts full ×7 coverage, verbatim EN mirror, no orphan keys and the joiner contract (vitest 837). Tier-1 drift maps seeded for all 915 new de/el/pl entries.

**Batch 33 (2026-09-02, thirty-third commit) — the ◇ review becomes a handoff.** With the register's executable work exhausted, its largest outstanding dependency — native review of every ◇ translation — was un-actionable: ~1,100+ branch-shipped strings per locale buried in 3,400-key catalogs. `scripts/i18n/export-native-review.mjs` (npm `i18n:export-review`) now diffs each catalog against the branch's merge-base with `origin/main` and emits `docs/i18n-review/{locale}.csv` — one row per added/edited string (`key, status, en, translation`; UTF-8 with BOM so Excel renders Greek/Hebrew) — plus a README with per-locale register guidance (de Sie, el case-neutral bestFor labels, pl nominative-by-design tags, he isolates/geresh, fr typography, ro diacritics) and the correction-return workflow. Generated and committed: de 1124 / el 1144 / pl 1126 / fr 1120 / he 1177 / ro 1118 rows (1,088 new keys each; the edited counts carry each locale's fix history — he's 89 include the batch-28 sweep). The script self-checks EN-source parity for every exported row. Also: AUD-107's row corrected — the "editorial project" it deferred to was in fact delivered as batch 28.

**Batch 34 (2026-09-02, thirty-fourth commit) — third review pass, 3 findings fixed.** A high-effort adversarial review of the batch 32–33 code surfaced three real defects, all remediated: (1) **the review export's blind spot** — it diffed only translations against the merge-base, so a string whose ENGLISH changed while the translation stayed put escaped the very sheets meant to catch staleness; 11 such rows existed, and four were semantically wrong live (the he guides-directory meta still claimed "verified hiking partners" after the branch's AUD-64 rewording to "licensed", the he book-guides meta still promised email confirmation the branch deliberately removed, and the de/pl directory metas lagged the same rewording) — the four translations are fixed, and the export now emits `source-changed` rows with an `en_was` column so the class can't hide again; (2) **Hebrew conjunction before Latin labels** — `formatBestForSentence` fused "ו" onto whatever came last, rendering "יום באקמס וThe Farmyard" on `/he/discover/sygkrasi` (the one Latin-kept label in the he catalog); it now uses the hyphenated "ו-" form with LTR isolates (the AUD-96 discipline) and adds a maqaf when a Latin-initial label lands first under the frame's fused "ל"; (3) **no freshness gate on the committed sheets** — any later catalog edit would silently stale them; the export gained a `--check` mode (skips itself when HEAD is the merge-base, so post-merge main runs don't false-fail) and CI's Quality job now runs it.

**Batch 35 (2026-09-02, thirty-fifth commit) — AUD-65/113 closed: invalid slugs answer real 404s.** The item had been parked as "needs a locale-detection redesign" after `dynamicParams=false` was experimentally disproven (the dynamic root layout commits the streaming shell before `notFound()` can set a status). The decisive datum arrived during batch-34 verification: a URL matching **no** route (`/he/guides`) returns a genuine HTTP 404 with the styled not-found page, dynamic layout notwithstanding. So the fix is a pre-render rewrite, not a redesign: `src/proxy.ts` (the existing next-intl + CSP chain) now consults `invalid-slug-404.ts` — "known family + unknown slug" rewrites onto a non-matching sub-path (`…/__404__`) before rendering starts. The valid-slug sets live in dependency-free `route-ids.generated.ts` (437 ids across discover/trails/book-winery/book-guide/wine-routes/regions/weather, mirroring each page's `generateStaticParams`; `npm run generate:route-ids` regenerates, `route-ids.test.ts` fails on data drift — the review-sheets discipline). Strictly fail-open: valid slugs, unknown shapes, and undecodable segments pass through untouched. Verified live across all 7 families in prefixed and unprefixed forms: invalid → HTTP 404 + styled page + full security headers (CSP nonce intact through the rewrite path); valid pages byte-stable. The locale-detection redesign drops off the backlog as a defect dependency — it remains only as optional architecture.
...
**Batch 38 (2026-09-02, thirty-eighth commit) — §5.2 closed: the trust pillar becomes honest.** With the user's decision in hand (honest page over real bios), the fabricated team roster is gone: `src/data/team.ts` deleted (and its `i18n:extract` hook removed), and `/team` — same route, same links, same SEO alternates — now carries **"How we choose places"** in all 7 locales: chosen-for-winter (November–March reality, not August photography), data validated on every change with cited claims, **verified-never-paid** partners (the badge means a confirmed contact route, nothing else; no pay-for-placement), and an honest corrections path. Nav label relabeled ×7 ("How we choose" / "Unsere Auswahl" / «Πώς επιλέγουμε» / "Jak wybieramy" / « Notre sélection » / «איך אנחנו בוחרים» / "Cum alegem"); `PRODUCT_DEEP.md`'s trust-source line now cites the transparent curation method instead of "team visibility". The page's claims are all mechanically true of this codebase (data:validate, citation validation, `isPartnerVerified` gating, trail condition reports). New strings ◇ pending native review; review sheets regenerated.

**Batch 39 (2026-09-02, thirty-ninth commit) — AUD-14 closed: the workation week keeps its own promise.** The template sold "Remote weekdays" and then booked a three-hour mountain hike plus a winery on a Thursday. Restructured (user-greenlit): weekdays 1–5 now carry exactly one nearby, after-work-sized stop (Cyprus Museum, the salt-lake flamingo walk, Governor's Beach, Leventis, a Lefkara evening) and the two-stop full days live on the weekend — Saturday Artemis + Tsiakkas, Sunday Paphos mosaics + Kolios. Description, seasonal note and booking note updated ×7 to describe the new shape ("one easy stop after work"; "weekday stops fit after a workday"; "book the Saturday winery slot early"). A new guard in `itinerary-templates.test.ts` enforces the invariant: days 1–5 ≤ 1 stop, weekend days > 1 — the promise can no longer silently regress. Nadia's Plan-stage cell finally matches her ICP.

**Batch 40 (2026-09-02, fortieth commit) — AUD-19 closed: one headline, seven languages.** The Greek, German and Polish home pages still opened with the previous copy generation ("Escape the cold…") while the other four locales carried "A quieter side of the island." — the audit's clearest single symptom of tier-1 locales living outside the drift gate. Unified (user-greenlit) on the current EN generation: «Η πιο ήσυχη πλευρά του νησιού.», "Die ruhigere Seite der Insel.", "Spokojniejsza strona wyspy." — and `home.headline` is now pinned in `editorial-{de,el,pl}.json`, so a future headline rewrite that skips a tier-1 locale fails the drift gate instead of shipping split-brained. Three strings, one closed decision, one new guard.

**Batch 41 (2026-09-02, forty-first commit) — guest-side cancel ships; the greenlit arc completes.** The last of the four user-greenlit decisions, and the one migration 008 was built for. `cancelBookingAsGuest` (`src/lib/bookings.ts`) mirrors the partner FSM — same transitions map, idempotent no-op, compare-and-set — but authorizes by booking id + the exact guest email the booking was created with, and an email mismatch answers the same `not_found` as a missing booking, so ids cannot be probed. `POST /api/bookings/[id]/cancel` (rate-limited under a new `booking-cancel` scope, Zod-validated) sends two emails only on a real transition: the guest's confirmation **in their stored locale** via the new `sendGuestCancellationEmail` (a distinct copy variant — the existing "cancelled" strings are partner-decline voice, wrong for a cancellation the guest chose) and an EN notice to the partner, gated by the same `isPartnerVerified` + `partnerEmail` rule as the request notifications. `/bookings` cards for pending/confirmed bookings gain a two-step inline cancel (confirm → POST → local-storage status flip per the AUD-74 lesson → SR announcement) ×7 locales. Five unit tests cover the localized send, anti-enumeration parity, idempotency, case-insensitive email match and validation; a live round-trip (create → wrong-email 404 → cancel → no-op) runs in the gate. The `/bookings` "cancel guidance" copy from AUD-09 finally points at a button that exists.

**Batch 42 (2026-09-02, forty-second commit) — fifth review pass: 10 findings in batches 37–41, all fixed.** The greenlit arc earned its own adversarial pass, which caught real gaps: (1) **cancel was silently unavailable cross-device** — the public API view strips `guestEmail`, so bookings synced via the emailed lookup link never showed the button; `applyRemoteBookings` now restores the queried email onto records that lack it (the server matched them by exactly that email); (2) the new public cancel endpoint read its body unbounded — now `readJsonBody` with a 4KB cap and a 413, like its siblings; (3) the AUD-09 `cancelHint` still told guests to "reply to your confirmation email or call" one line above the new in-app button — reworded ×7 to point at the card (and the stale BUG-362 comment corrected); (4) the confirm dialog promised "{provider} will be notified" though the notice only goes to verified partners — the promise is dropped ×7 rather than overclaimed; (5) the booking FSM existed as two hand-synced 40-line copies — now one `transitionBookingStatus` core with an authorize callback, partner and guest paths as thin wrappers; (6) the partner notice's plain-text subject was HTML-escaped ("Anne &amp; Tom") — unescaped; (7) `handleCancelled` now updates React state from memory before best-effort storage, so a failed localStorage write can't announce success over a stale list; (8) §0.0/§0.1 updated for the arc's own closures (the stale-summary class batch 37 existed to prevent); (9) a timestamp-only `vc-excursions-index.json` churn that rode batch 38 reverted; (10) the cancel 200 now carries `rateLimitSuccessHeaders` (Cache-Control: no-store on PII, X-RateLimit-\*) per the helper's contract.

**Batch 43 (2026-09-03, forty-third commit) — AUD-25's executable half: every locale gets its own advisory authority.** The travel-trust strip (home footer, `/plan`, `/bookings`) sent all seven locales to gov.uk — a German traveler was told to consult the British government while the Auswärtiges Amt publishes its own Zypern security page, and the Hebrew locale ignored that Israel's National Security Council runs a dedicated Cyprus travel-warnings page (the IL-01 crisis scenario verbatim). The finding was parked only for lack of *verified* sources; sourcing research closed that: `src/lib/advisory-links.ts` maps each locale to its verified authority — FCDO (en), Auswärtiges Amt (de), France Diplomatie (fr), MSZ (pl), MAE `travel-conditions/3682` (ro), NSC `gov.il/he/pages/cyprus1` (he) — and, since Greece publishes no per-country advisories, el links the Greek MFA's Cyprus mission page (consular help for Greeks on the island). The map is typed `Record<Locale, string>` (compile-time completeness), guarded by a unit test (locale coverage vs `routing.locales`, https, distinct hosts, `Object.hasOwn` fallback), and the label key is renamed `travelTrust.foreignTravelUk` → `travelTrust.advisory` with each label naming its own authority ×7 (the historical BUG-313 overrides record updated so a re-run can't resurrect the old key). Alert-time behavior and the `/book/*` strip placement remain open as non-code halves.

**Batch 44 (2026-09-03, forty-fourth commit) — sixth review pass: the index-churn class is root-caused, not re-reverted.** Batch 43's commit again carried timestamp-only churn in `vc-cycling-index.json` and `vc-excursions-index.json` — the same class batch 42 had reverted once. This pass found the mechanism instead of the symptom: the intake scripts end with module-scope `void main()`, and their vitest files import pure helpers from them, so **every `npm run test` executed the scripts' CLI path on import** — a network fetch (egress-blocked here, hence `count: 0`) followed by a `writeFileSync` that re-stamped the committed JSON mid-test-run (the churn timestamps match the batch-43 vitest start to the second). Three siblings shared the landmine with worse payloads: `vc-trail-gap.ts` ends in `process.exit()` (a successful fetch would have killed the test worker), `vc-hiking-map-validate.ts` printed its full validation report into every test run, and `fetch-vc-trail-index.ts` would rewrite its own index. All five now guard `main()` with the `isCliEntry` idiom the repo already used in `parse-tourist-guides-pdf.ts`; the two churned files are reverted to their pre-batch-43 stamps; the targeted suites pass with `git status` clean afterward — the regression proof the class never had — and the validators still print when run directly. Also recorded for honesty: the six advisory URLs from batch 43 cannot be live-verified from this environment (all six government domains are egress-blocked, gov.uk included); they stand on the search-index attestation gathered during batch-43 planning.

**Batch 45 (2026-09-03, forty-fifth batch commit) — the gate learns to see AUD-02: populated-plan visual guard.** The §1 scorecard's Test & CI row measured the visual gate's blind spots at audit time, and the sharpest one was how Critical AUD-02 shipped: every `/plan` check in `e2e/visual-qa-gate.spec.ts` ran against an **empty plan**, so a title column collapsing to 24×216px (one letter per line) at the funnel's payoff moment was invisible to a green gate. The AUD-02 fix itself had no regression guard — a future layout change could re-break it exactly the way it broke the first time. Two new gate tests close that: `/plan?add=lefkara` (the audit's measured case, populated via the same `?add=` mechanism `plan-share.spec.ts` uses) and `/he/plan?add=lefkara` (AUD-02 reproduced "in English and Hebrew alike") locate the itinerary title by its locale-proof `a[href$="/discover/lefkara"]` and bound its box — width ≥ 100px (broken was ~24), height ≤ 120px (broken was ~216) — plus actions-row presence and the overflow check. The visual gate is now 19 tests; the empty-plan blind spot from the audit's own scorecard is closed.

**Batch 46 (2026-09-03, forty-sixth batch commit) — the last named gate blind spot: detail routes join the overflow sweep.** The §1 Test & CI row measured three visual-gate blind spots at audit time; plural rules (AUD-42) and the stale GF6 claim (AUD-03) closed in round 1, batch 45 closed the empty-plan check, and this batch closes the third: **detail routes were never overflow-checked** — the @375 sweep covered only hubs, the two existing detail tests asserted hero images without an overflow call, and the chain's 320px sweep skips detail routes too, which is exactly how AUD-15's 5px overflow lived on *every* discover detail (LTR and RTL) under a green gate. Now `/book/winery/tsiakkas` and `/discover/tsiakkas` end with `expectNoHorizontalOverflow`, `/trails/artemis` gets its own @375 detail check, and `/he/discover/tsiakkas` guards the RTL side of the class. Visual gate: 19 → 22 tests. All three audit-named Test & CI blind spots are closed.

**Batch 47 (2026-09-04, first post-merge commit) — AUD-23's executable half: an honest guide price signal.** PL-01's budget lens went unanswered because no per-guide prices exist to show — and fabricating them was never on the table. The same sourcing move that unlocked AUD-25 applies: the Cyprus Tourist Guides Association's indicative rates are published (via FEG, the European guides federation): €100–150 per half-day, €150–400 per full day. The guide booking hub, every guide detail page, and the plan guide bar now carry that range ×7 locales — explicitly "indicative", attributed to the association, and closed with "your guide confirms the final quote", so the signal answers the budget question without promising a price nobody set. Hebrew euro ranges ride the repo's LRI…PDI isolate convention. One shared key (`book.pages.guideRateRange`) plus a short plan-bar variant (`plan.guideBar.rateHint`); the fourteen new strings ride the `docs/i18n-review/` handoff. True per-guide quotes remain with the guides — that half of AUD-23 stays supply.

**Batch 48 (2026-09-04, second post-merge commit) — AUD-81's executable half: the honest onboarding surface.** B2B-01 had no page anywhere explaining how a winery or guide gets listed. `/partner/join` now exists ×7 locales, mirroring the `/team` trust-page pattern (server-rendered card grid, Strategy-A alternates): what a listing is (free, editorial, verified-never-paid — the same claims `/team` makes), who can join (Deputy-Ministry-licensed guides; genuinely winter-open wineries), how verification is earned (the badge means a confirmed license *and* a booking contact that actually reaches the partner — the `partner-verification.ts` contract in plain words), and how to start today (existing partners → portal; licensed guides already surface from the official register; the direct onboarding contact "opens soon" — the page deliberately publishes **no** address while the AUD-67 mailbox doesn't exist, extending the no-dead-channels rule). Entry links: the partner-portal sign-in ("Not a partner yet?") and the `/team` page footer. The onboarding *contact channel* remains supply-gated on the real mailbox.

**Batch 49 (2026-09-04, third post-merge commit) — seventh review pass: `/partner/join` joins the route registries.** Batches 47–48 earned the same adversarial pass every prior arc got, and it found two instances of one class — the new public route was missing from the static-route registries: `src/app/sitemap.ts` (every public static page is listed, `/team` included; an indexable page invisible to the sitemap is the SEO-hygiene class the audit policed) and the `e2e/route-smoke.spec.ts` static list (the net that catches exactly the 404 class batch 48 hit mid-gate). Both fixed — a sitemap entry at 0.4/monthly beside `/team`, and a route-smoke row so a future regression on the route goes red in CI. Everything else in the 47–48 diff re-read clean: the `[locale]` mirror matches the house pattern, the meta entry is correct, Hebrew ranges carry real LRI…PDI isolates, sheet counts reconcile. Running total: seven review passes, 36 self-caught defects.

**Batch 50 (2026-09-04, on the branch restarted from the PR #232 merge) — AUD-80 residual: the server names the failing field; stale-doc sweep.** The register's last deferred booking-form item: a server-side `VALIDATION_ERROR` reached guests as a banner-only generic while every input stayed unmarked. Both ends of the plumbing already existed and had never been connected — `jsonError` accepts `details[{field,message}]` (unused), and the forms already render `fieldErrors` with `aria-invalid`, `aria-describedby` and an inline message (client-Zod-only). Now `/api/bookings` names the failing field from the first Zod issue (and from the sanitize-to-empty `guestName` guard), and `useBookingForm` maps a field the form actually renders into `fieldErrors` with a localized inline marker ×7 (`errors.serverField`, both form namespaces) and moves focus to that input instead of scrolling the banner; a field the form doesn't render keeps today's banner-only behavior, and the server's EN-only message text still never surfaces. Guarded by two route tests (`details[0].field` names `partySize`/`guestName`) and two hook tests (map+focus; unknown-field no-op). The rider closed the stale-doc debt the audit flagged in §5/§6.1 but never applied: `GOLDEN_FLOWS_AUDIT.md`'s GF6 row now records the AUD-03 regression and fix (and its mangled literal-`\n` block is repaired), `DEEP_REVIEW_2026-05-29.md`'s "~43% English" claims carry superseded notes, and `QA_BUGS.md` drops its two stale rows (G2 overlay — closed by migration 009; team roster — closed by batch 38). Two product decisions recorded: the advisory strip stays off `/book/*` by design (AUD-25 half closed), and `/install` becomes a guest-facing PWA install guide (batch 51). The secret-gems localization arc (last AUD-10 class, 78 entries) is approved as batches 52+.

**Batch 51 (2026-09-04) — AUD-66 closed: `/install` becomes the guest "save the app" guide.** The route had been a SiteGround shared-hosting deployment manual shipped public in 7 locales — sitemap-hidden since the audit, but still the wrong content at a public URL. The app underneath is a genuinely complete PWA (per-locale manifests with standalone display and shortcuts, a hand-written network-first service worker, maskable icons, a custom install banner with an iOS Share-hint fallback), so the page now says that to guests instead: a benefits card whose claims are limited to what the code actually does (visited pages keep working offline; bookings and live weather stay online-only), three platform cards (Android/Chrome, iPhone-iPad/Safari mirroring the existing `pwa.prompt.iosHint` wording, desktop Chrome/Edge with the honest "Safari and Firefox don't offer this"), and an honest closing note — on the `/partner/join` template, with no `t.rich` and no placeholders so the BUG-350 crash class cannot recur. The key tree shrank 37 → 17 ×7 (the dead `troubleshooting.items.images.config` key retired with the rest; all six non-EN trees ◇ pending native review). With guest content at the URL, the page rejoined the public-page registry class batches 49–50 policed: the `robots` override left `translated-page-meta.ts` (test updated), both `robots.ts` disallows dropped, and the sitemap entry returned at 0.3/monthly — route-smoke and the axe sweep already covered the route. Rider: `src/app/apple-icon.png` (from the existing 192px icon) so iOS home screens stop scaling the favicon.

**Batch 52 (2026-09-04, on the branch restarted from the PR #233 merge) — the fourth and final content class opens: secret gems go native.** The 78 curated "insider tip" entries — the register's last English-only content class — get the same message-overlay machinery as wineries, attractions and trails: `src/lib/secret-gem-content.ts` overlays `data.secretGems.{id}.{title,body}` onto the base record at render, coverage is explicit in `secret-gem-content-ids.ts`, and a mirrored guard test asserts every covered field is non-empty in all 7 catalogs with EN mirroring the data verbatim. All three render surfaces are wired per-request — the `/secrets` hub (all 78 cards), discover detail and trail detail (`getSecretsForPlace`) — so future slices are zero-code, exactly like the earlier classes. The pilot covers the 13 gems attached to flagship template/discover places (Kourion's bench, the Omodos kafenion, Kato Lefkara, Nissi's winter café, the Avakas pairing, the Tombs of the Kings timing tip…), keeping the clipped insider voice in each register — *Sie*-German, plural-imperative Greek and Hebrew (clock times written as words, so no bidi digits), informal Polish and Romanian, *vous*-French — with local toponyms where natives use them (Κούριον, Μυλλομέρης, קברי המלכים). 26 keys ×7 (catalog 7×3,445), tier-1 drift maps seeded, all ◇ pending native review. Coverage: 13 of 78; batches 53+ take the remaining 65 on the zero-code path.

**Batch 53 (2026-09-04) — gems slice 2: the next 22, zero code.** The overlay machinery proves itself the way the earlier classes did — the slice touches only the coverage registry, the catalogs and the drift maps. The 22 gems in data order (Zambartas–Aes Ambelis pairing, Bellapais' Tree of Idleness, Louvaras and Lofou before Omodos, Leventis-before-Cyprus-Museum, the Ledra buffer-zone crossing, Kouklia's café, the Polis loop, St Hilarion's Queen's Window, Kolossi's free sugar mill, Governor's coves, misty Choirokoitia, Fikardou's square, the Madari fire tower, rainy-day Nicosia, Zygi's fish tavernas, Laiki Geitonia, quiet Vavla, Foini's potters, Kritou Terra, Maronite Kormakitis, Statos) ship ×6 in the same registers — Durrell's Tree of Idleness reads natively in seven languages, the north-Cyprus caveats (car needed, not on the tourist loop) keep their teeth, winery brand names stay Latin while villages take native forms (Λόφου, Χοιροκοιτία, קורמקיטיס). 44 keys ×7 (catalog 7×3,489), drift maps seeded, sheets at 70 rows/locale. Coverage: 35 of 78.

**Batch 54 (2026-09-04) — gems slice 3: the wine-and-food twenty-two.** The next 22 in data order are the practical ones — winery visit mechanics (Zambartas' barrel sample, Tsiakkas at 1,460 m, Vasilikon's no-appointment hours, Vlassides' Rainbow tasting, snowbound Kyperounta, the Atypon range, Tsangarides' century, Vouni Panayia's comparative Commandaria) and the meal pairings hikers actually use (Psilo Dendro's trout, Polis and Latsi harbours, The Farmyard, Kakopetria's psaróvrasto, Zygi Sundays, Troodos Square's post-hike warmth, Cape Greco then Konnos). Figures follow the batch-47 conventions — "20 €"/"7,50 €" in de/el/pl/fr/ro, Hebrew keeping "€20" with LRI…PDI isolates on dash ranges (⁦€5–14⁩, ⁦8:30–18:00⁩) — and opening hours/prices carry over exactly from the EN base so nothing is invented. 44 keys ×7 (catalog 7×3,533), drift maps seeded, sheets at 114 rows/locale. Coverage: 57 of 78; the final 21 land in batch 55.

**Batch 55 (2026-09-04) — the final 21 gems: the AUD-10 register is complete.** The last slice carries the outdoors set — Tzelefos bridge at dawn, the Ineia kafenion road-intel for climbers, Pomos pier snorkelling, two-night Tochni, Asinou's 1105 frescoes before the museum, remote Vretsia with its "tell someone your route", Green Bay's turtles, the Episkopi winter-sun crag, the Amphorae Caves' look-don't-touch, Amiantos' honest industrial heritage, the Araka church key at Lagoudera, Tenta before Lefkara, calm-morning Mackenzie, the Orange Valley ride, Eptagoneia's kafenion beta, Athalassa's garden-then-lake — in all six non-EN registers, safety caveats intact. 42 keys ×7 (catalog 7×3,575), drift maps seeded, sheets at 156 rows/locale. **Coverage: 78 of 78. With it, the audit's #1 RICE project closes end to end: 71 wineries, 58 attractions, 105 trails and 78 secret gems — the entire curated content register — render natively in all 7 locales, every field guard-tested, everything ◇ awaiting native review.**

**Batch 56 (2026-09-05, on the branch restarted from the PR #234 merge) — the arc reviews itself; the register's blind spot gets a name.** The eighth adversarial pass, over batches 50–55, found four real bugs, three minor gaps and four documentation drifts — all fixed. The real ones: (1) the batch-50 server-named field marker *destroyed itself on the next blur* — the client schema is looser than the server's (email `max(320)` vs `max(254)`), so blurring the unchanged rejected value passed client Zod and wiped the only visible error; `useBookingForm` now remembers the server-flagged field with the exact rejected value and refuses to clear it until the value changes (hook-test-pinned). (2) `knownFields` was built from *request* field names, so a server detail naming `notes` or `trailId` would focus an input that renders no error — each form now declares its own `errorFields` contract (the four inputs that actually render inline errors). (3) Network-failure detection matched English message text (`/failed to fetch|network error/i`) that WebKit never produces ("Load failed") — on offline Safari the booking was never queued and the raw English hit the banner in all 7 locales; detection is now by type (`TypeError` / `navigator.onLine`), and only messages the hook localized itself may render (a non-JSON 502's `SyntaxError` falls back to the localized generic). (4) 15 pre-convention Hebrew strings carried unisolated numeric ranges rendering reversed (24–48 reads as 48–24 — among them the booking SLA ×4, the weather meta description search engines show, both difficulty tips); all now wrapped in LRI…PDI like the 138 already-correct ones, with a new guard test that scans the whole he catalog so the class is pinned. Minor: `/install` was an orphan (sitemap-listed, zero internal links) — the iOS install banner now links the guide and the footer carries it (`nav.install`/`footer.install`/`pwa.prompt.guideLink` ×7, catalog 7×3,578); the breadcrumb uses a proper short label; `/secrets`' sr-only index heading no longer announces over the empty state. Docs: this section's final-state header, PR list, catalog size and batch order corrected — and one substantive repair: the review-sheet export was merge-base-scoped, so the PR #233→#234 branch restart silently dropped 19 still-◇ keys from every sheet; the export now pins the audit baseline (`main@ca827e6`), and the sheets hold the branch's full review surface (~1,350 rows/locale) permanently. Rider (value sweep): `/beaches`, `/villages` and `/cycling` rendered `AttractionCard`s without the AUD-10 overlay, silently discarding translations that already shipped — all three now localize per-request like `/discover` (zero new strings; all 7 beaches were covered the whole time). The same sweep named the register's blind spot — `i18n:scan` skips `src/data/`, leaving weather/airport/events/restaurants/nature/cycling-route/wine-route prose EN in localized frames — queued as the next arc (§0.0).

**Remaining backlog (decision/supply/project):** 67 (real legal mailbox), 72 (legal "last updated"), 81-residual (onboarding contact channel — the honest `/partner/join` page shipped batch 48),  AUD-23 residual (per-guide quotes — the association-range signal shipped batch 47), AUD-25 residual (alert-time crisis behavior — needs an alert source; the `/book/*` strip closed by design, batch 50), AUD-58 (family-lane curation — editorial), 104-residual (≤360px cap by design), 121-residual (161–185-char metas, accepted), `useTrapFocus` consolidation (deferred, not a defect), native review of all ◇ translations shipped on this branch (handoff package ready: `docs/i18n-review/` — batch 33).

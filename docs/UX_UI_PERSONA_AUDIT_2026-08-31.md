...
## 0.0 Final state (2026-09-04, after batch 50)
...
Where the branch ended up, 50 batches later (batches 1–46 merged in PR #227, 47–48 in PR #231, 49 in PR #232; batch 50 continues on the restarted branch):
...
| AUD-66 | Med | `/install` is a developer deployment guide shipped as a public 7-locale route; `sitemap.ts` listed it while robots+meta forbid indexing | ◐ sitemap entry removed; decision recorded (batch 50): rewrite as a guest-facing PWA install guide — scheduled as batch 51 |
...
**Remaining backlog (decision/supply/project):** 67 (real legal mailbox), 72 (legal "last updated"), 81-residual (onboarding contact channel — the honest `/partner/join` page shipped batch 48), AUD-66 `/install` guest rewrite (decision recorded batch 50; scheduled batch 51), AUD-10 secret-gems localization (arc approved batch 50; batches 52+ — winery, attraction and trail classes are complete), AUD-23 residual (per-guide quotes — the association-range signal shipped batch 47), AUD-25 residual (alert-time crisis behavior — needs an alert source; the `/book/*` strip closed by design, batch 50), AUD-58 (family-lane curation — editorial), 104-residual (≤360px cap by design), 121-residual (161–185-char metas, accepted), `useTrapFocus` consolidation (deferred, not a defect), native review of all ◇ translations shipped on this branch (handoff package ready: `docs/i18n-review/` — batch 33).

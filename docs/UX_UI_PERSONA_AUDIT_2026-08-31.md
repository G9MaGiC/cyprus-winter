...
## 0.0 Final state (2026-09-04, after batch 49)
...
Where the branch ended up, 49 batches later (batches 1–46 merged in PR #227, 47–48 in PR #231; batch 49 continues on the restarted branch):
...
- **Still open, none code-shaped:** real legal mailbox (AUD-67), legal "last updated" (AUD-72), the partner onboarding *contact channel* (AUD-81/67 residual — the honest `/partner/join` page shipped batch 48; a real mailbox still gates the direct channel), per-guide price quotes (AUD-23 residual — the honest association-range signal shipped batch 47), family-lane curation (AUD-58), AUD-25's remaining halves (alert-time crisis behavior needs an alert source; strip on `/book/*` is a form-clutter design call), the rest of the §5 known-open ledger (partner reachability, email domain, supply-gated content, photos, legal sign-off), secret-gems copy if that class is ever added, and the native ◇ review itself.
...
| AUD-25 | Crisis posture is a static UK-centric strip: gov.uk advisory link hard-coded for all 7 locales; no alert-time behavior; strip absent from /book/* — **✅ batch 43 (link-map half):** each locale now links its own verified authority (Auswärtiges Amt / France Diplomatie / MSZ / MAE / Israel's NSC Cyprus page / Greek MFA Cyprus mission; EN keeps the FCDO) via `src/lib/advisory-links.ts` + `travelTrust.advisory` ×7; alert-time behavior (needs an alert source) and strip-on-`/book/*` (a form-clutter design call) stay open | `[S]` `src/components/travel/TravelTrustStrip.tsx:30` | CRISIS-01, IL-01 | y (per-locale link map) |
...
**Remaining backlog (decision/supply/project):** 67 (real legal mailbox), 72 (legal "last updated"), 81-residual (onboarding contact channel — the honest `/partner/join` page shipped batch 48), AUD-10 secret-gems editorial copy if that class is ever added to the data (winery, attraction and trail classes are all complete), AUD-23 residual (per-guide quotes — the association-range signal shipped batch 47), AUD-25 residuals (alert-time crisis behavior — needs an alert source; strip on `/book/*` — design call), AUD-58 (family-lane curation — editorial), 104-residual (≤360px cap by design), 121-residual (161–185-char metas, accepted), `useTrapFocus` consolidation (deferred, not a defect), native review of all ◇ translations shipped on this branch (handoff package ready: `docs/i18n-review/` — batch 33).

...
**Captured:** 31 August 2026 against `https://cyprus-winter-three.vercel.app/api/health`  
**`cypruswinter.com`:** not attached to Vercel (DNS). Recapture against the custom domain once DNS is live.
...
HTTP status was **503**. That matches the health route when required production env is missing (Upstash Redis + Supabase). Pages such as `/` and `/discover?view=map` still returned **200** on the same host.
...
```bash
curl -sS "https://cyprus-winter-three.vercel.app/api/health" \

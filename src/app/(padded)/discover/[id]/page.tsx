        ...
        <article aria-label={tDetail("articleAria", { name: a.name, type: a.type, region: a.region })}>
          ...
          <section className={`${CARD.base} ${CARD.contentLg} ${CALLOUT.cta}`}>
            ...
            {a.combineWith && a.combineWith.length > 0 && (
              <RelatedPlacesBlock
                ids={a.combineWith}
                description={tDetail("combineWithDesc")}
                showAddToItinerary
              />
            )}
            ...
            {(() => {
              ...
              const typeKey = (["winery", "restaurant", "beach", "ancient", "village", "monastery", "nature"] as const).includes(a.type as never)
                ? a.type : "default";
              const typeLabel = tDetail(`typeLabels.${typeKey}`);
              return (

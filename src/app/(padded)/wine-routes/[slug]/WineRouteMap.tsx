...
export default function WineRouteMap({ routeTitle, center, wineries }: WineRouteMapProps) {
  ...
  return (
    <section aria-labelledby="wine-route-map-heading" className="mt-10">
      <h2 id="wine-route-map-heading" className={`${TYPE.kicker} text-muted-ink mb-3`}>
            ...
            <Marker key={w.id} position={[w.latitude!, w.longitude!]} icon={wineryIcon}>
              <Popup>
                <div className="min-w-[200px]">
                  ...
                  <p className="text-xs text-muted-ink mb-3">{w.region}</p>
                  <AppLink
                    href={`/discover/${w.id}`}
      ...
      <p className="mt-2 text-xs text-muted-ink">{tPage("map.caption")}</p>
    </section>
  );
}

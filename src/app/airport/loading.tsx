import { LAYOUT, CARD } from "@/lib/design-tokens";

export default function AirportLoading() {
  return (
    <div
      className={`min-h-screen bg-sand ${LAYOUT.listNarrow} mx-auto ${LAYOUT.safeAreaX} ${LAYOUT.pagePy}`}
      aria-busy
      aria-live="polite"
      role="status"
      aria-label="Loading airport guide"
    >
      <div className="animate-pulse">
        <div className="h-4 w-24 bg-sand-200/80 rounded mb-4" />
        <div className="h-9 w-56 bg-olive/20 rounded mb-2" />
        <div className="h-4 max-w-md bg-sand-200/80 rounded mb-8" />
        <div className="h-14 w-full rounded-xl bg-aegean/10 border border-aegean/20 mb-8" />
        <div className="flex gap-3 mb-10">
          <div className="h-11 w-16 bg-sand-200/80 rounded-lg" />
          <div className="h-11 w-16 bg-sand-200/80 rounded-lg" />
        </div>
        <div className="space-y-10">
          {[1, 2].map((i) => (
            <div key={i} className={`overflow-hidden ${CARD.base}`}>
              <div className="h-20 bg-terracotta/20" />
              <div className={CARD.content}>
                <div className="h-5 w-24 bg-olive/20 rounded mb-4" />
                <div className="space-y-3">
                  {[1, 2, 3].map((j) => (
                    <div key={j} className="h-16 bg-sand-100 rounded-lg" />
                  ))}
                </div>
                <div className="h-5 w-28 bg-olive/20 rounded mt-6 mb-3" />
                <div className="space-y-2">
                  <div className="h-4 w-full bg-sand-200/80 rounded" />
                  <div className="h-4 w-4/5 bg-sand-200/80 rounded" />
                </div>
              </div>
            </div>
          ))}
        </div>
        <div className={`mt-10 ${CARD.content} rounded-xl bg-sand/80 border border-sand-200/80`}>
          <div className="h-5 w-28 bg-olive/20 rounded mb-4" />
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-4 w-full bg-sand-200/80 rounded" />
            ))}
          </div>
        </div>
      </div>
    </div>
  );
}

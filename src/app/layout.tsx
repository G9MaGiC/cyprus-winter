...
export const metadata: Metadata = {
  ...
  description:
    "Cyprus winter guide: Troodos trails, wineries, villages. Trail conditions, tastings, events. Sixteen degrees when home is six. Plan or explore.",
        ...
        <footer role="contentinfo" aria-label="Site footer" className={`border-t border-sand-200 bg-sand-100/80 py-10 ${LAYOUT.safeAreaX} pb-[max(calc(5rem+env(safe-area-inset-bottom)),1.5rem)] md:pb-[max(1.5rem,env(safe-area-inset-bottom))]`}>
          <div className={`${LAYOUT.listNarrow} mx-auto text-center`}>
            <nav aria-label="Essentials" className="flex flex-wrap justify-center gap-6 text-sm font-medium text-olive/80 mb-4">
              <Link href="/airport" className="hover:text-terracotta transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-terracotta/50 rounded">Arriving</Link>
              <Link href="/weather" className="hover:text-terracotta transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-terracotta/50 rounded">Weather</Link>
              <Link href="/bookings" className="hover:text-terracotta transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-terracotta/50 rounded">Bookings</Link>
            </nav>
            <p className="text-sm text-olive/70 prose-body max-w-md mx-auto break-words">
              Whether you found us from a Google search or at the airport—trails, villages, wine. Emergency 112 · Tourist info 1460 · Ambulance 199.
            </p>

...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  // Booking URLs mirror the page predicates (launch-truth hardening): only
  // wineries that can receive a request and only public guides are listed.
  const wineryBookingPages: MetadataRoute.Sitemap = wineries
    .filter((w) => w.isBookable !== false && Boolean(w.bookingUrl))
    .map((w) => entry(base, `/book/winery/${w.id}`, 0.6, "monthly"));

  const guideBookingPages: MetadataRoute.Sitemap = [
    entry(base, "/book/guide", 0.6, "monthly"),
    ...guides
      .filter((g) => g.isPublic !== false)
      .map((g) => entry(base, `/book/guide/${g.id}`, 0.6, "monthly")),
  ];

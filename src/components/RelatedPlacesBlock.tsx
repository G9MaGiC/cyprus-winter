  ...
  return (
    <section className={`${CARD.base} ${CARD.contentLg} bg-sand-100/90`}>
      ...
      <ul className="flex flex-wrap gap-3">
        {related.map((r) => (
          <li key={r.id} className="min-w-0 flex flex-wrap items-center gap-2.5">
            <AppLink
              href={r.href}
              className="group inline-flex items-center min-h-[44px] gap-2 px-5 py-3 rounded-2xl bg-sand-100/80 border border-sand-200/80 text-olive font-medium text-sm hover:bg-terracotta-muted hover:text-white hover:border-terracotta/30 transition-colors duration-150 max-w-full min-w-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-terracotta/50 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
            >

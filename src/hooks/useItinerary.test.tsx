...
describe("useItinerary", () => {
  ...
  it("can merge current storage before adding from independent same-tab instances", async () => {
    ...
    await waitFor(() => {
      ...
      expect(stored["1"]).toContain("kourion");
    });
    ...
    await waitFor(() => {
      const stored = JSON.parse(localStorage.getItem(STORAGE_KEY) ?? "{}") as Record<string, string[]>;
      expect(stored["1"]).toEqual(expect.arrayContaining(["kourion", "pafos-mosaics"]));
    });
  });

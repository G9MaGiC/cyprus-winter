...
describe("useItinerary", () => {
  ...
  it("addToDayIfMissing can merge latest storage before persisting same-tab card adds", async () => {
    ...
    await waitFor(() => {
      const parsed = JSON.parse(localStorage.getItem(STORAGE_KEY)!) as Record<string, string[]>;
      expect(parsed["1"]).toEqual(expect.arrayContaining(["kourion", "pafos-mosaics"]));
    });
  });

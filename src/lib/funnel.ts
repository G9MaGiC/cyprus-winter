...
type FunnelEventRow = { event: string | null };
type FunnelSourceRow = { event: string | null; properties: unknown };
...
export async function getEventSourceBreakdownInRange(
  ...
  const { data, error } = await fetchAllSupabaseRows<FunnelSourceRow>(() => {
    let query = supabase
      .from("conversion_events")

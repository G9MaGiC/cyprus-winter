...
const MAX_ITEMS = 50;
let processingQueue: Promise<{ processed: number; succeeded: number }> | null = null;
...
export function processQueue(): Promise<{ processed: number; succeeded: number }> {
  if (processingQueue) return Promise.resolve({ processed: 0, succeeded: 0 });
  processingQueue = processQueueOnce().finally(() => {
